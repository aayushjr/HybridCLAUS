"""
2D Capsule implementation from Duarte et al. (Visual-textual capsule routing for text-based video segmentation)
...
class CapsNet(nn.Module):
    ...
    def forward(self, img, classification, latent=False):
        '''
        INPUTS:
        ...
        if latent:
            ...
            combined_caps = combined_caps.permute(0, 3, 1, 2)
            pooled_acts = F.max_pool2d(combined_caps, kernel_size=combined_caps.size()[2:])
            pooled_acts = pooled_acts.reshape(-1, combined_caps.shape[1])
            return out_1, actor_prediction, pooled_acts
        else:
